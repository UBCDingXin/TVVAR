...
L_tau = 10
tau_tvvar_grid = seq(0.0001, 0.005, length.out=L_tau)
...
#data processing
#########################################
exchange_with_time=exchange_rate
exchange_rate[,1]=NULL

# stock50.ix = c(1,2,7,4,5,6,13,9,10,21);
stock50.ix = c(3,4,6,7,8,9,11,20);
stock50.data = exchange_rate[,stock50.ix]
...
{
  # rawX[,i]=detrend(X[,i], tt = 'linear',bp=round((nrow(X)-T_test)*b))
