...
weight_epanechnikovker <- function(t, m, n, b)
{
  ...
  else
  {
    ...
    # #Triweight
    # numerator = (35/32) * (1 - v^2)^3
    # vi=sapply(1:n,function(x){(t-x/n)/b})
    # indx=abs(vi)<=1
    # denumerator=sum((35/32)*(1-vi[indx]^2)^3)
    # return(numerator/denumerator)
    
  }
  # numerator = dnorm((t-m/n)/b)
  # vi=sapply(1:n,function(x){dnorm((t-x/n)/b)})
  # denumerator=sum(vi)
  # return(numerator/denumerator)
}
...
# -----------------------------------------------------------------------------
# fit a Ridge_VAR model based on LS with l2 penalty
# statlsvar_fit is a function who fit a stationary Ridge_VAR model.
# there are 2 inputs: 
# X is a T*d array which stores the data of the time series. T is the length and d is the dimension.
# lam is the shrinkage parameter

statlsvar_fit<-function(X,lam){
  T=dim(X)[1]
  d=dim(X)[2]
  W1=crossprod(X[2:T,],X[1:(T-1),])
  W2=crossprod(X[1:(T-1),],X[1:(T-1),])
  A_hat_statLS=tcrossprod(W1,chol2inv(chol(W2+lam*diag(d))))#W2 is symmetric
  return(A_hat_statLS)
}


# -----------------------------------------------------------------------------
# fit a stat Lasso_VAR model based on LS with l1 penalty
# statlavar_fit is a function who fit a stat Lasso_VAR model.
# there are four inputs: 
# X is a T*d array which stores the data of the time series. T is the length and d is the dimension.
# lam is the shrinkage parameter
# alpha is the stepsize
# A_old is the initial matrix of A_k and A_oldold is A_{k-1}
statlavar_fit<-function(X,lam,alpha,A_old,A_oldold){
  # alpha=1/max(svd(crossprod(X,X))$d)
  T=dim(X)[1]
  d=dim(X)[2]
  W1=crossprod(X[2:T,],X[1:(T-1),])
